...
class Tenant():
    ...
    def init_add(self, args):
        ...
        if "physical_tpm" in data:
            logger.info("SILVIOOOOO Siamo dentro")
            self.physical_tpm = data["physical_tpm"]
    ...
    def check_ek(self, ekcert, physical_tpm = 1):
        ...
        if physical_tpm is not None and physical_tpm == 0:
            logger.info("Skipping ekcert check due to the absence of a Physical TPM") 
            return True
    ...
    def validate_tpm_quote(self, public_key, quote, hash_alg):
        ...
        if not self.check_ek(reg_data['ekcert'], reg_data['physical_tpm']):
            return False
        # if agent is virtual, check phyisical EK cert and make sure it matches phyiscal EK
        if 'provider_keys' in reg_data:
            if not self.check_ek(reg_data['provider_keys']['ekcert'], reg_data['physical_tpm']):
                return False
    ...
    def do_cv(self):
        ...
        data = {
            ...
            'physical_tpm' : self.physical_tpm
        }
...
def main(argv=sys.argv):
    ...
    #SILVIOOO
    #parser.add_argument('-z', '--physicaltpm', action='store', dest='physical_tpm', default='False',
     #                   help="used to define if the agent has a physical tpm or not")
