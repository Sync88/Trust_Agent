        ...
        try:
            ...
            if agent_id is not None:
                ...
                if content_length == 0:
                    ...
                    agent_data['physical_tpm'] = json_body['physical_tpm']
...
async def invoke_get_quote(agent, need_pubkey):
    ...
    #SILVIOOOOOO : add physical_tpm to the url 
    res = tornado_requests.request("GET",
                                   "http://%s:%d/v%s/quotes/integrity?nonce=%s&mask=%s&vmask=%s&partial=%s&ima_ml_entry=%d&physical_tpm=%s" %
                                   (agent['ip'], agent['port'], version, params["nonce"], params["mask"], params['vmask'], partial_req, params['ima_ml_entry'], agent['physical_tpm']), context=None)
    response = await res
    ...
    try:
        ...
        if new_operational_state in (states.FAILED, states.INVALID_QUOTE):
            ...
            for key in exclude_db:
                if key in agent:
                    del agent[key]
            if agent['physical_tpm']:
                del agent['physical_tpm']
            session.query(VerfierMain).filter_by(
