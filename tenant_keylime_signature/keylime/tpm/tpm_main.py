...
from keylime import cryptodome
...
class tpm(tpm_abstract.AbstractTPM):
    ...
    #Silvioooooooooooo: add last 3 parameters
    def _tpm2_checkquote(self, aikTpmFromRegistrar, quote, nonce, hash_alg, signature, ima_measurement_list, physical_tpm = None):
        ...
        try:
            ...
            if ima_measurement_list is not None and physical_tpm == 0:
                ret = cryptodome.rsa_verify(cryptodome.rsa_import_pubkey(aikFromRegistrar), ima_measurement_list.encode(), signature) 
                if not ret:
                    logger.error("IMPOSSIBLE TO VERIFY THE SIGNATURE OVER THE MEASUREMENT LIST")
                    return None, False
    ...
    #SILVIO: add parameters (signature and physical_tpm)
    def check_quote(self, agentAttestState, nonce, data, quote, aikTpmFromRegistrar, tpm_policy={}, ima_measurement_list=None, allowlist={}, hash_alg=None, ima_keyring=None, mb_measurement_list=None, mb_refstate=None, signature=None, physical_tpm = None):
        if hash_alg is None:
            hash_alg = self.defaults['hash']
        #SILVIO: add parameters(signature, ima_measurement_list and physical_tpm)
        retout, success = self._tpm2_checkquote(aikTpmFromRegistrar, quote, nonce, hash_alg, signature, ima_measurement_list, physical_tpm)
        if not success:
            return success
        ...
        return self.check_pcrs(agentAttestState, tpm_policy, pcrs, data, False, ima_measurement_list, allowlist, ima_keyring, mb_measurement_list, mb_refstate, physical_tpm)
